(() => {
  ...
  const priorityVideos = new Set(videos.filter(video => video.hasAttribute('data-priority-video')));
  ...
  const pauseVideo = video => {
    if (!priorityVideos.has(video)) video.pause();
  };
  if ('IntersectionObserver' in window) {
    ...
    videos.forEach(video => {
      if (priorityVideos.has(video)) startVideo(video);
  ...
  works.forEach(work => {
    ...
    if (video) work.addEventListener('mouseenter', () => startVideo(video), { passive: true });
  });
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) videos.forEach(video => {
      if (priorityVideos.has(video) || video.getBoundingClientRect().top < window.innerHeight + 800) startVideo(video);
    });
  });
